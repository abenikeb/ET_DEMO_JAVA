package com.et.pwsdemo.config;

/**
 * Demo Config
 */
public interface PWSConfig {

    String FabricAppId = "c4182ef8-9249-458a-985e-06d191f4d505";
    String AppSecret = "fad0f06383c6297f545876694b974599";
    String MerchantAppId = "930231098009602";
    String MerchantCode = "101011";
    String MerchantPrivateKey = "";
    String TestPhoneNumber = "251922092001";

    String BaseUrl = "https://196.188.120.3:38443/apiaccess/payment/gateway";

}